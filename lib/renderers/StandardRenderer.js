...
var defaultConfig = require('bower-config/lib/util/defaults');
...
StandardRenderer.prototype.error = function (err) {
    ...
    if (this._config.verbose || !err.code || err.errno) {
        ...
        var curr = currentConfig();
        for(property in defaultConfig) {
            if(defaultConfig.hasOwnProperty(property)) {
                if(curr[property] != defaultConfig[property]) {
                    console.log("not equal: "+property+" -> "+curr[property]+" : "+defaultConfig[property]);
                }
            }
        }
